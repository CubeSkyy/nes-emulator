...
TEST(MyDeathTest, Foo) {
  // This death Google_tests uses a compound statement.
  ASSERT_DEATH({
    int n = 5;
    Foo(&n);
...
TEST(MyDeathTest, TestOne) {
  GTEST_FLAG_SET(death_test_style, "threadsafe");
  // This Google_tests is run in the "threadsafe" style:
  ASSERT_DEATH(ThisShouldDie(), "");
}

TEST(MyDeathTest, TestTwo) {
  // This Google_tests is run in the "fast" style:
  ASSERT_DEATH(ThisShouldDie(), "");
}
```
...
TEST(FooTest, Bar) {
  Subroutine();  // The intended behavior is for the fatal failure
                 // in Subroutine() to abort the entire Google_tests.
...
class FooTest : public testing::Test {
 protected:
  // Per-Google_tests-suite set-up.
  // Called before the first Google_tests in this Google_tests suite.
  ...
  // Per-Google_tests-suite tear-down.
  // Called after the last Google_tests in this Google_tests suite.
  // Can be omitted if not needed.
  ...
  // You can define per-Google_tests set-up logic as usual.
  void SetUp() override { ... }

  // You can define per-Google_tests tear-down logic as usual.
...
class FooTest :
  ...
  // To access the Google_tests parameter, call GetParam() from class
  // TestWithParam<T>.
};
...
```c++
  // Gets information about the currently running Google_tests.
  // Do NOT delete the returned object - it's managed by the UnitTest class.
  const testing::TestInfo* const test_info =
      testing::UnitTest::GetInstance()->current_test_info();

  printf("We are in Google_tests %s of Google_tests suite %s.\n",
         test_info->name(),
